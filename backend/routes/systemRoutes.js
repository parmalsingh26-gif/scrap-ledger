...
import { Category, Unit, Item } from '../models/BaseData.js';
import { Inward, Outward, InventoryBalance } from '../models/Inventory.js';
import { BvpScrapEntry, BvpCoachEntry, BvpSurveyEntry } from '../models/BvpScrap.js';
...
router.post('/bvp/init', async (req, res) => {
  const scrapCount = await BvpScrapEntry.countDocuments();
  const coachCount = await BvpCoachEntry.countDocuments();
  const surveyCount = await BvpSurveyEntry.countDocuments();
  ...
  try {
    if (scrapCount === 0) await BvpScrapEntry.insertMany(SEED_SCRAP, { ordered: false }).catch(e => console.log('Scrap seed dup skip'));
    if (coachCount === 0) await BvpCoachEntry.insertMany(SEED_COACH, { ordered: false }).catch(e => console.log('Coach seed dup skip'));
    if (surveyCount === 0) await BvpSurveyEntry.insertMany(SEED_SURVEY, { ordered: false }).catch(e => console.log('Survey seed dup skip'));
  } catch (err) {
